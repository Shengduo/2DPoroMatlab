function Rc_diffusion_factors_p_control_elastic_1_narrow(Nuu, Nu, ... 
                    Gamma, cc, ...
    ...
    d0 = (heaviside(-x + 2).*heaviside(x + 2))';
    d00 = d0 == 0;
    ...
    for it = 2:NT
        ...
        % Elastic case %% IMPORTANT: NOT IMPLEMENTED FOR PRESSURE CONTROL
        else
        ...
        while tryagain || enter
            ...
            else %% IMPORTANT: NOT IMPLEMENTED FOR PRESSURE CONTROL
                RHS = pcp - (bfs - bns)./(bfp + bnp) .* (siyy - siyyp) ...
                      - (dphi - dphip) ./ (phi*(bfp + bnp)) ...
                      + (INjectmass(t+dt) - INjectmass(t)) * INprofile./(rhof0*phi*(bfp + bnp)) ...
                      ;
            ...
            if tryagain == 0
                ...
                if rem(it,NSplot)==0 || it == 2 || (runnerplot > 1 && max(V./Vsave(:,runnerplot - 1)) > 10)
                    ...
                    InjectMaSave(runnerplot) = INjectma; % INjectmass(t+dt); 
                    if plotflag == 1
                        f1 = figure(1);
